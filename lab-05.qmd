...
download.file('https://zenodo.org/records/15529996/files/camels_attributes_v2.0.pdf?download=1',
              '../ESS_523c_lab1/data/camels_attributes_v2.0.pdf',
              mode = 'wb')
```
...
zero_q_freq is frequency of days with Q = 0 mm/day 
...
wf_try <- workflow_set(list(rec), list(lm_model, rf_model,b_model,nn_model)) %>%
  workflow_map('fit_resamples', resamples = camels_cv) 

autoplot(wf_try)

rank_results(wf_try, rank_metric = "rsq", select_best = TRUE)

```

> Based the neural network model has the highest ranking and R2, but, as stated in class, neural networks are often considered data hungry and opaque. For this reason I would probably chose to use random forest instead. This model ranked second with metrics that were not remarkably different from the neural network. This type of model is advantageous because it reduces overfitting and requires minimal hyperparameter tuning.
...
dist_plot_multi <- function(ds = camels, pred_select) {ds_long <- ds %>%
    pivot_longer(cols = all_of(pred_select),
                 names_to = "variable",
                 values_to = "value")

  ggplot(ds_long) + geom_histogram(aes(x = value), bins = 20) + facet_wrap(~variable, scales = "free") +theme_bw()
}

dist_plot_multi(ds = camels,
  pred_select = c("q_mean", "p_mean", "soil_porosity", "pet_mean", "elev_mean", "frac_forest","slope_mean")
)

 
```

> Variables that have missing values include geol_porosity, root_depth_50, and root_depth_99. There are a number of other variables with just 1 missing reported including q_mean. A few plots have a right skew including q_mean,slope_mean, mean elevation. Forest fraction has a distribution that is split peaking near 0 and 1.

### Geography Check (optional)

```{r}
#| echo: false

#Geography Check 
## High Moran's I = candidate geographic proxy to revisit in the VIP step.

coords<-cbind(camels$gauge_lon, camels$gauge_lat)
dist_mat <- as.matrix(dist(coords))
W <- 1 / dist_mat
diag(W) <- 0   
W[is.infinite(W)] <- 0

moran_select = c("p_mean", "soil_porosity", "pet_mean", "elev_mean", "frac_forest","slope_mean")

moran_results <-map_dfr(moran_select, function(v) {
  res <- Moran.I(camels[[v]], W)
  tibble(
    variable = v,
...
moran_results %>% arrange(desc(observed))
...
cor_all<-camels |> 
  select( q_mean, p_mean, soil_porosity,pet_mean, elev_mean, frac_forest, slope_mean) |> 
  drop_na() |> 
  cor()
cor_all_df<-as.data.frame(cor_all)
...
```{r}
#From Notes: The default is 0.75, which means 75% of the data will be used for training and 25% for testing.

#strata = ...,

camel_strata<-initial_split(camels, prop = 0.75)

camel_train<-training(camel_strata)
camel_test<- testing(camel_strata)
  
  
camel_cv <- vfold_cv(camel_train, v = 10)
```

> I did not chose to strata by a varaible
...
#offset to avoid log0

rec2<-recipe(logQmean~p_mean+frac_forest+pet_mean+slope_mean+soil_porosity, data = camel_train) %>% 
  step_log(slope_mean,pet_mean,p_mean, offset = 1) %>% 
   step_mutate(frac_forest = asin(sqrt(frac_forest))) %>%
...
> \
> I chose P_mean and pet_mean over aridity since aridity is PET/P so including all three may introduce multicollinearity which would make it harder to see the impact of these variables on the model. P_mean is understandably going to be an important predictor of streamflow. Forest_frac can inform transpiration and runoff. Slope and porosity may inform the runoff making it to the stream. Slope, pet_mean, and p_mean had a log transformation step applied to it because their distributions showed some right skew. All variables were then normalized to put them on a common scale and an interaction between p_mean and soil_porosity. Finally a step was created to remove observations containing na.
...
```{r}
rf_model2<-rand_forest() %>% set_engine("ranger",importance = "impurity") %>% set_mode("regression")

b_model2<-boost_tree() %>% set_engine("xgboost") %>% set_mode("regression")

svm_model <- svm_poly() %>% 
  set_engine("kernlab") %>% 
  set_mode("regression")

lm_model2<-linear_reg() %>% set_engine("lm") %>% set_mode("regression")

```

> Random forest captures any nonlinear relationships and interactions without upholding strong assumptions. Boost tree (XGBoost) is computationally efficient and includes regularization. These typically have small decision trees, but these smaller trees prevent overfitting. The support vector machine model was chosen because it can model nonlinear relationships in 'high-dimensional spaces' to solve complex, non-linear problems. I also added a simple linear model as a good baseline to capture simple relationships.
...
#spatial
camel_train_sf<-camel_train %>% st_as_sf(coords=c("gauge_lon","gauge_lat"), crs=4326, remove = F)

spatial_cv<-spatial_block_cv(camel_train_sf, v=10)
...
wf_2cv<-workflow_set(preproc = list(base=rec2),
                   models = list(rf_model2,b_model2, svm_model, lm_model2))%>%
  workflow_map('fit_resamples', resamples =camel_cv)

wf_2sp<-workflow_set(preproc = list(base=rec2),
                   models = list(rf_model2,b_model2, svm_model, lm_model2))%>%
...
rank_results(wf_2cv, rank_metric = "rsq", select_best = TRUE)
rank_results(wf_2sp, rank_metric = "rsq", select_best = TRUE)


```

> Random forest model performs best under both random and spatial. Under CV this model has a rsq of 0.896 and a rmse of 0.396. This compares to spatial CV where values decreased: rsq of 0.874 and a rmse of 0.400. This small drop is expected and implies that the model doesn't overly rely on spatial factors, but is still capturing these relationships.
...
rf_model_tune<-rand_forest(mtry=tune(),min_n = tune(), trees = 500) %>% set_engine("ranger",importance = "impurity") %>% set_mode("regression")
...
> I chose to tune the random forest based on the ranking of the models. I chose to tune the hyperparameters of mtry and min_n. I chose grid values mtry based on the number of predictors & interaction in my model. The model appears to be most sensitive to the mtry hyperparameter, a number for the number of predictors that will be randomly sampled at each split when creating the tree models, and less sensitive to min_n, a minimum number of data points in a node that are required for the node to be split further. The optimal value for this appears to be 3. After 3 selected predicitors the rsq and rmse taper off.
...
rsqbest<-select_best(rf_tuning,metric = "rsq")
...
rf_fit <- fit(rf_final, data = camel_train)
...
> Precipitation is the most important variable to predict discharge. This is expected because more precipitation means more water in the stream. The next most important predictor is fraction of forest. More trees can transpire more water compared to bare land. Additionally, slope was found to be important; a higher mean slope means more runoff guiding the water to the stream. ET, and porosity had less importance in this model. There is evidence of some spatial autocorrelation, but none of these predictors have Moran's I values calculated above 0.5, therefore it is unlikely they are acting as geographic proxies.
...
final_results <- last_fit(rf_final, split = camel_strata)

metrics <- collect_metrics(final_results)

# rmse and rsq
rmse <- round(metrics$.estimate[metrics$.metric == "rmse"], 3)
rsq  <- round(metrics$.estimate[metrics$.metric == "rsq"], 3)


camel_test_full <- final_results %>%
  extract_workflow() %>%
  augment(new_data = testing(camel_strata))

# untransform dis
camel_test_full$q_mean_pred <- exp(camel_test_full$.pred)
camel_test_full$q_mean_obs  <- exp(camel_test_full$logQmean)



ggplot(camel_test_full, aes(x = q_mean_pred, y = q_mean_obs, color = aridity)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, linetype = 2, color = "black") +
  scale_color_viridis_c(option = "plasma") +
  labs(
    title = "Observed vs Predicted Streamflow (Test Set)",
    subtitle = paste("RMSE:", rmse, "| R2:", rsq),
    x = "Predicted Q (mm/day)",
    y = "Observed Q (mm/day)",
    color = "Aridity"
  ) +
  theme_bw()
...
> This model seems to preform well with an R2 of 0.886 and a rmse of 0.38, illustrated by the points following the line on the plot. The higher flow values have more spread, especially for one outlier near 10 mm/day suggesting this model is not as accurate for extreme events. These extremes all have low aridity values so it may have been smart to considered stratification in data splitting. Compared to spatial CV value of R2 0.874 and rmse of 0.400 these values are similar so it was a good estimate. If I were going to investigate further, I would look at different climate regions.\
